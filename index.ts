...
if (hideFilterButton) hideFilterButton.addEventListener('click', hideFilters);

const titleSort = getTargetElement('title-filter', document.getElementsByTagName('select'));
titleSort?.addEventListener('change', (e) => sortBooks(e as SelectChangeEvent));

const yearSort = getTargetElement('year-filter', document.getElementsByTagName('select'));
yearSort?.addEventListener('change', (e) => sortBooks(e as SelectChangeEvent));

const authorFilter = getTargetElement('author-filter', document.getElementsByTagName('select'));
authorFilter?.addEventListener('change', (e) => filterBooks(e as SelectChangeEvent, 'author'));

const pagesSort = getTargetElement('pages-filter', document.getElementsByTagName('select'));
pagesSort?.addEventListener('change', (e) => sortBooks(e as SelectChangeEvent));

const genreFilter = getTargetElement('genre-filter', document.getElementsByTagName('select'));
genreFilter?.addEventListener('change', (e) => filterBooks(e as SelectChangeEvent, 'genre'));
...
function isStringRecordsArray(data: unknown): data is StringRecords[] {
...
function getTargetElements<T extends HTMLElement>(className: string, tagsList: HTMLCollectionOf<T>): T[] {
...
function getBooksData() {
  ...
  const parsedBookData: unknown = storageBookData ? JSON.parse(storageBookData) : null;
...
function createErrorMessage(text: string) {
...
function removeErrorMessage(element: HTMLElement) {
...
function signUpUser() {
  ...
  const adminData: unknown = JSON.parse(localStorage.getItem('Admin') ?? '');
  ...
  if (loginInput?.value && passwordInput?.value) {
    const isAdmin =
      parsedAdminData?.some((admin) => admin.login === loginInput.value && admin.password === passwordInput.value) &&
      overlayModalContent &&
      overlaySignUpModal;

    const isUser =
      parsedUserData?.some((user) => user.login === loginInput.value && user.password === passwordInput.value) &&
      overlayModalContent &&
      overlaySignUpModal;
...
function createCard(bookData: StringRecords, isPreview?: boolean) {
...
function createUserPanel(dataBook?: StringRecords[]) {
  if (titleHeader) titleHeader.innerText = 'User panel';
  ...
  if (!filterPanel?.classList.contains('hidden')) showFiltersButton.classList.add('hidden');
  else showFiltersButton.classList.remove('hidden');

  if (userContent && filterPanel) userContent.insertBefore(containerCards, filterPanel);
}

function createItem(bookData: StringRecords) {
...
function editBook(key: string) {
  ...
  if (!targetBook || !textarea || !addBookButton) return;
...
function restoreBook(key: string, restoreButton: HTMLImageElement, deleteButton: HTMLDivElement) {
...
function previewBook() {
  ...
  if (textarea) book['description'] = textarea.value;

  const card = createCard(book, true);
  if (adminContentWrapper) adminContent?.insertBefore(card, adminContentWrapper);
}

function createAdminPanel() {
  if (titleHeader) titleHeader.innerText = 'Add new book';
...
function checkModal() {
  ...
  if (!textarea?.value) textPanel[textPanel.length - 1].classList.add('error');
  else textPanel[textPanel.length - 1].classList.remove('error');

  if (inputs.some((input) => !input.value) || !textarea?.value) {
    for (let i = 0; i < inputs.length; i++) {
      if (!inputs[i].value) textPanel[i].classList.add('error');
      else textPanel[i].classList.remove('error');
    }
    ...
    if (isErrorExists) adminContentWrapper.prepend(errorMessageDiv);
    return false;
